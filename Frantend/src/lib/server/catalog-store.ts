...
function normalizeTemplate(template: Partial<ProposalTemplate>) {
  const fallback = defaultTemplates.find((item) => item.id === template.id);

  return {
    id: template.id ?? fallback?.id ?? "",
    proposalId: template.proposalId ?? fallback?.proposalId ?? 0,
    relationshipType: template.relationshipType ?? fallback?.relationshipType ?? "GF",
    name: template.name ?? fallback?.name ?? "",
    tagline: template.tagline ?? fallback?.tagline ?? "",
    description: template.description ?? fallback?.description ?? "",
    family: template.family ?? fallback?.family ?? "romantic",
    image: template.image ?? fallback?.image ?? "",
  } satisfies ProposalTemplate;
}

function normalizeTemplates(templates: Partial<ProposalTemplate>[]) {
  return templates
  ...
  try {
    ...
    return {
      templates: Array.isArray(parsed.templates) && parsed.templates.length > 0
        ? normalizeTemplates(parsed.templates)
...
export async function getTemplates() {
  try {
    ...
    return Array.isArray(response.templates) && response.templates.length > 0
      ? normalizeTemplates(response.templates)
      : defaultStore.templates;
  } catch {
...
export async function saveTemplates(templates: ProposalTemplate[]) {
  const store = await readStore();
  store.templates = normalizeTemplates(templates);
  await writeStore(store);
  try {
    await backendRequest("/site-state/templates", {
      method: "PUT",
      body: JSON.stringify({ templates: store.templates }),
    });
  } catch {}
  return store.templates;
}
...
export async function upsertTemplate(template: ProposalTemplate) {
  ...
  const index = store.templates.findIndex((item) => item.id === normalizedTemplate.id);

  if (index === -1) {
    store.templates.push(normalizedTemplate);
  } else {
    store.templates[index] = normalizedTemplate;
  }
  ...
  try {
    await backendRequest("/site-state/templates", {
      method: "POST",
      body: JSON.stringify({ template: normalizedTemplate }),
    });
  } catch {}
