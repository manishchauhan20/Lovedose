...
export function TemplateCatalog({
  ...
  const grouped = useMemo(
    () => ({
      GF: templates.filter((item) => item.relationshipType === "GF"),
      Wife: templates.filter((item) => item.relationshipType === "Wife"),
      Crush: templates.filter((item) => item.relationshipType === "Crush"),
    }),
    [templates],
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      {(Object.keys(grouped) as RelationshipType[]).map((type) => (
        <section
          key={type}
