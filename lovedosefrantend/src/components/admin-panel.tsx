...
import { TEMPLATE_RENDERER_PRESETS, getDefaultRendererKeyForFamily } from "@/lib/template-renderers";
...
const emptyTemplate: ProposalTemplate = {
  id: "",
  relationshipType: "GF",
  name: "",
  tagline: "",
  ...
  family: "romantic",
  image: "",
  rendererKey: getDefaultRendererKeyForFamily("romantic"),
};
...
export function AdminPanel() {
  ...
  const uploadTemplateImage = async (file: File) => {
    await runAdminAction("upload-template-image", async () => {
      ...
      const data = await response.json();
      if (!response.ok) return void setStatus(data.error || "Image upload failed.");
      setTemplateForm((f) => ({ ...f, image: data.url || "" }));
      setStatus("Template cover image uploaded.");
    });
  };
              ...
              <div className="flex-1 overflow-y-auto px-4 pb-5 flex flex-col gap-1.5">
                {templates.map((t) => (
                  <button key={t.id} type="button" onClick={() => handleTemplateSelect(t.id)} className="w-full text-left px-3 py-3 rounded-xl transition-all" style={selectedTemplateId === t.id
                    ...
                    <p className="text-white/18 text-[0.55rem] mt-1 uppercase tracking-[0.1em]">{t.relationshipType} · {t.family} · {t.rendererKey || getDefaultRendererKeyForFamily(t.family)}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 pt-5 pb-10 min-w-0">
              <div className="max-w-2xl flex flex-col gap-4">
                ...
                >
                  ...
                  <p className="text-white/40 text-[0.58rem] tracking-[0.18em] uppercase relative z-10">{templateForm.relationshipType} · {templateForm.family} · {(templateForm.rendererKey || getDefaultRendererKeyForFamily(templateForm.family))}</p>
                ...
                <div className="p-5 rounded-2xl" style={{ background: "#0d0b08", border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <AdminSelect label="Family" value={templateForm.family} options={["romantic","royal","dreamy"]} onChange={(v) => setTemplateForm((f) => ({ ...f, family: v as ProposalTemplate["family"], rendererKey: f.rendererKey || getDefaultRendererKeyForFamily(v as ProposalTemplate["family"]) }))} />
                    <AdminSelect label="Relationship" value={templateForm.relationshipType} options={[...RELATIONSHIP_TYPES]} onChange={(v) => setTemplateForm((f) => ({ ...f, relationshipType: v as ProposalTemplate["relationshipType"] }))} />
                  </div>
                  <div className="mb-3">
                    <AdminFileUpload
                      label="Cover Image"
                      value={templateForm.image}
                      buttonLabel={actionLoading === "upload-template-image" ? "Uploading..." : "Upload Cover"}
                      disabled={actionLoading === "upload-template-image"}
                      onFileSelect={(file) => void uploadTemplateImage(file)}
                      onClear={() => setTemplateForm((f) => ({ ...f, image: "" }))}
                    />
                  </div>
                  <div className="grid grid-cols-1 gap-3 mb-3">
                    <AdminSelect
                      label="Renderer Preset"
                      value={TEMPLATE_RENDERER_PRESETS.includes((templateForm.rendererKey || "") as (typeof TEMPLATE_RENDERER_PRESETS)[number]) ? (templateForm.rendererKey || "") : ""}
                      options={["", ...TEMPLATE_RENDERER_PRESETS]}
                      onChange={(v) => setTemplateForm((f) => ({ ...f, rendererKey: v || f.rendererKey || getDefaultRendererKeyForFamily(f.family) }))}
                    />
                    <AdminInput label="Renderer Key" value={templateForm.rendererKey || ""} onChange={(v) => setTemplateForm((f) => ({ ...f, rendererKey: v }))} />
                  </div>
                  <p className="text-white/30 text-[0.7rem] leading-relaxed mb-4">
                    `Renderer Key` ab `Praposal-section` ke andar relative path hoga. Example:
                    `Crush-Collection/proposal-template3` ke liye file
                    `src/components/Praposal-section/Crush-Collection/proposal-template3.tsx`.
                    Naya renderer banana ho to folder ke andar file banao, default export do, phir yahi path save karo.
                  </p>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <label
          className={`px-3 py-2.5 rounded-xl text-sm transition-all ${disabled ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", color: "#f0e8d8" }}
        >
          <input
            type="file"
        ...
        {value && (
          <button
            type="button"
            onClick={onClear}
            className="px-3 py-2.5 rounded-xl text-xs tracking-[0.1em] uppercase text-red-400/70 hover:text-red-400"
            style={{ border: "1px solid rgba(255,100,100,0.12)" }}
          >
            Remove
          </button>
        )}
      ...
      >
        {value ? (
          <div className="flex items-center gap-3 p-3">
            ...
            <p className="text-white/45 text-xs break-all">{value}</p>
          </div>
        ) : (
          <div className="px-3 py-4 text-white/25 text-xs">No cover image selected.</div>
        )}
      </div>
    </div>
  );
